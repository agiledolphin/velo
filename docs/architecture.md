...
站点封面加载与代表帧生成并行进行。首次媒体解析会从 yt-dlp 结果中选择不高于 480p 的最佳视频流，将直链与请求头写入 Rust 内存中的限量 LRU 缓存；缓存最多保留 8 条并在 10 分钟后过期，直链不会进入 WebView。代表帧生成优先复用缓存，缺失或过期时才以固定参数再次调用 yt-dlp。FFmpeg 仅允许 HTTP/HTTPS 及其必要传输协议，只转发白名单请求头并拒绝换行注入，通过输入侧快速定位从约 1 秒处输出最大宽度 640 的 JPEG data URL。成功时代表帧替换站点封面；超时、无视频流、直链失效或输出无效时不影响解析结果，并继续使用站点封面或品牌占位图。
...
FFmpeg 与 yt-dlp 使用相同的目标三元组 sidecar 机制，但拥有独立资产清单。macOS 与 Linux 固定到 Martin Riedl FFmpeg 8.1.2 的版本化 URL，Windows 固定到 Gyan FFmpeg 8.1.2 Essentials 的 GitHub release；各平台均对 ZIP 和解压后二进制分别验证硬编码 SHA-256。ZIP 解压只处理清单指定的可执行文件，构建脚本限制下载大小并原子替换 sidecar，发布工作流进一步检查 NSIS 与 DEB 内同时包含两个可执行文件。第三方来源与许可提示作为 Tauri resource 随包分发；正式发布前仍需完成完整许可审查。
