...
站点封面加载与代表帧生成并行进行。代表帧生成器再次以固定参数调用 yt-dlp，选择不高于 480p 的视频流；解析出的直链只保留在 Rust 进程内。FFmpeg 仅允许 HTTP/HTTPS 及其必要传输协议，只转发白名单请求头并拒绝换行注入，在约 1 秒处输出最大宽度 640 的 JPEG data URL。成功时代表帧替换站点封面；超时、无视频流、直链失效或输出无效时不影响解析结果，并继续使用站点封面或品牌占位图。
