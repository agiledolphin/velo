...
React 主界面使用内存态页签管理多个单视频任务。每个页签挂载独立的
`MediaInspector`，因此地址、解析状态、媒体信息、格式选择、保存位置与下载进度不会
在切换时丢失；非活动页签只隐藏面板，不卸载正在运行的任务。

页签最多开放 8 个，并继续保持一个页签只接受一个 URL。页签关闭会卸载对应解析器，
由现有清理逻辑取消仍在运行的解析请求；活动下载期间禁止关闭页签，用户必须先通过任务
自身的取消入口终止下载，避免后端留下没有界面订阅者的活动任务。设置仍为应用级视图，
不会复制到单个页签。

解析请求与下载任务继续使用独立 ID。后端已有的活动任务映射可同时管理多个页签，前端
根据任务 ID 过滤全局下载事件，不需要新增批量命令，也不解除 yt-dlp 的
`--no-playlist` 约束。8 个页签是工作区容量，不等同于外部进程并发数：前端解析调度器
使用独立的 2 槽 FIFO 队列，下载协调器使用另一个 2 槽队列，因此两类任务互不阻塞。
等待任务可取消，活动任务完成、失败或取消后自动启动队首任务。页签活动状态当前不会
写入磁盘；重启恢复留待后续里程碑。

成功下载历史与活动任务状态分开保存。前端只在收到带新序号的 `completed` 事件后写入
版本化的 WebView 本地存储，按任务 ID 去重并最多保留最近 50 条；失败、取消和等待任务
不会进入历史。记录包含标题、站点、格式、容器、完成时间和目标路径，可在设置页查看或
经二次确认后整体清空。该轻量记录不承担任务恢复职责，后续若引入可恢复任务数据库，
应迁移现有版本化记录而不是复用 Cookie 的 `settings.json`。

默认下载目录属于应用配置，保存在 `settings.json` 的 `downloads` 命名空间。未配置时
使用操作系统下载目录；自定义目录必须是已存在的绝对目录，设置快照同时返回有效性，
目录失效后不能静默回退。默认下载任务由 Rust 使用安全文件名生成目标路径，并在已有
文件后依次添加 `(1)`、`(2)` 等序号；前端不拼接路径。用户仍可通过原生“另存为”
对话框为单个任务覆盖位置，两种入口都会在目标验证后直接启动下载。
...
应用设置使用带 `schemaVersion` 的结构化 JSON 保存在 Tauri 应用配置目录，站点配置位于 `sites` 命名空间，便于以后独立增加站点策略。Cookie 文件只持久化绝对路径，不复制文件内容；读取和保存均通过类型化 Tauri 命令完成，不向前端开放任意配置键写入。

YouTube Cookie 默认采用按需模式。解析引擎先发起匿名请求，仅在错误被分类为身份验证要求时使用已验证的 Netscape Cookie 文件重试一次。Cookie 参数只会为 YouTube、YouTube Music、`youtu.be` 与隐私增强域名生成；其他站点即使共享同一个 yt-dlp 实例也不会收到该参数。按需重试成功的来源会在有限内存集合中标记，使随后代表帧刷新和下载沿用同一认证上下文；修改 Cookie 配置会清空这些标记。
...
`DownloadCoordinator` 在单进程内登记活动任务，拒绝重复任务 ID，并为每个任务持有独立取消通道。协调器最多向下载引擎放行 2 个任务；新任务先发送 `queued`，取得信号量许可后才发送 `started`。排队期间取消会直接发送 `cancelled`，不会启动 yt-dlp；运行中的取消则终止并回收对应子进程。任务在完成、失败或取消后释放许可，使 FIFO 队首自动补位。
